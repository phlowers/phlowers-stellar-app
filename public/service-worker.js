...
async function installApp() {
    ...
    cache.put('app_version', new Response(JSON.stringify(buildVersion), {
        headers: {
...
async function updateApp() {
    ...
    await cache.put('app_version', new Response(JSON.stringify(appVersion), {
        headers: {
            'content-type': 'application/json'
        }
    }));
...
async function handleFetch(event) {
    ...
    if (url === scope) {
        ...
        event.respondWith(caches.match(newUrl).then((response) => {
            if (response) {
                return response;
            }
