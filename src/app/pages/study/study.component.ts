...
@Component({
  selector: 'app-admin',
  standalone: true,
  imports: [ButtonModule, ProgressSpinnerModule, CommonModule, TableModule, InputTextModule, FormsModule],
  template: `<div>
    <div class="pb-5">
      ...
      <ng-template #body let-support let-editing="editing">
        <tr>
          ...
          <td [pEditableColumn]="support.suspension" pEditableColumnField="suspension">
            <p-cellEditor>
              <ng-template #input>
                <input pInputText type="boolean" [(ngModel)]="support.suspension" />
              </ng-template>
              <ng-template #output>
                {{ support.suspension }}
              </ng-template>
