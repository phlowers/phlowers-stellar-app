...
@Component({
  standalone: true,
  imports: [ThreeDModalComponent, CreateSectionComponent, SearchSectionModalComponent, PopoverModule, CommonModule, FormsModule, ButtonModule, InputTextModule, TextareaModule, InputNumberModule, DialogModule, TableModule, ToolbarModule],
  template: `
    <app-create-section (save)="onSaveSection($event)" (cancel)="onCancelSection()"></app-create-section>

    <p-dialog dismissableMask="true" [style]="{ width: '80vw', height: '90vh' }" header="3D View" [(visible)]="threeDModalOpen" (onHide)="closeThreeDModal()" [modal]="true">
      <app-three-d-modal />
    </p-dialog>
    ...
    <p-toolbar styleClass="mb-6">
      <ng-template #start>
        <div class="flex flex-row gap-2">
          <p-button label="Search" icon="pi pi-search" severity="secondary" (onClick)="search()" />
          <p-button label="Create" icon="pi pi-plus" severity="secondary" (onClick)="openCreateSectionDialog()" />
        </div>
      </ng-template>
    </p-toolbar>
    <p-table
...
export class SectionsComponent implements OnInit {
  @ViewChild(CreateSectionComponent) createSectionDialog!: CreateSectionComponent;
