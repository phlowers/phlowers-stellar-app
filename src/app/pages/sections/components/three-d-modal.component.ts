...
@Component({
  selector: 'app-three-d-modal',
  standalone: true,
  styleUrls: ['./three-d-modal.component.scss'],
  imports: [ButtonModule, DialogModule, ProgressSpinnerModule, CommonModule, TableModule, InputTextModule, FormsModule, CheckboxModule, NgxSliderModule, AutoCompleteModule, CheckboxModule],
  template: `
    <div>
      <p-button label="Run Python" (onClick)="runPython()"></p-button>
      <!-- <p-button label="log" (onClick)="log()"></p-button> -->
      <div class="my-5">
        <p-autocomplete [(ngModel)]="selectedPhase" [dropdown]="true" [suggestions]="phases()" (completeMethod)="search($event)" />
      </div>
      <div class="flex items-center my-5">
        <p-checkbox inputId="showOtherAsDashed" [binary]="true" [(ngModel)]="showOtherAsDashed"></p-checkbox>
        <label for="showOtherAsDashed" class="ml-2">Show other phases as dashed</label>
      </div>
      <div class="my-10">
...
export class ThreeDModalComponent {
  ...
  async createPlot(minValue: number, maxValue: number, selectedPhase: string, showOtherAsDashed: boolean, currentCamera: Camera | undefined) {
    ...
    const uniqueSupportsForSupports = uniq(Object.values(lit.support)).slice(minValue, maxValue + 1);
    ...
    const phase1 = selectedPhase === 'all' || selectedPhase === 'phase_1' || showOtherAsDashed ? this.getAllPhases(litXs, litYs, litZs, litCanton, litTypes, litSupports, uniqueSupports, 'phase_1') : [];
    const phase2 = selectedPhase === 'all' || selectedPhase === 'phase_2' || showOtherAsDashed ? this.getAllPhases(litXs, litYs, litZs, litCanton, litTypes, litSupports, uniqueSupports, 'phase_2') : [];
    const phase3 = selectedPhase === 'all' || selectedPhase === 'phase_3' || showOtherAsDashed ? this.getAllPhases(litXs, litYs, litZs, litCanton, litTypes, litSupports, uniqueSupports, 'phase_3') : [];
    const gard = selectedPhase === 'all' || selectedPhase === 'garde' || showOtherAsDashed ? this.getAllPhases(litXs, litYs, litZs, litCanton, litTypes, litSupports, uniqueSupports, 'garde') : [];
