...
describe('WorkerService', () => {
  ...
  let mockWorker: { postMessage: jest.Mock; onmessage: ((ev: { data: Record<string, unknown> }) => void) | null };
  ...
  beforeEach(() => {
    ...
    (global as unknown as Record<string, unknown>).Worker = jest.fn().mockImplementation(() => mockWorker);

    // Mock URL constructor
    (global as unknown as Record<string, unknown>).URL = jest.fn().mockImplementation(() => 'mocked-url');
