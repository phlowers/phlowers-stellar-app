...
describe('ScaleViewComponent', () => {
  ...
  describe('Template — data-testid', () => {
    ...
    describe('Popover form content (opened)', () => {
      ...
      it('slider should have aria-label "Nombre de points par portée"', () => {
        const slider = document.body.querySelector('[data-testid="scale-slider"]');
        expect(slider?.getAttribute('aria-label')).toBe('Nombre de points par portée');
      });
      ...
      it('input number should have aria-label "Nombre de points par portée"', () => {
        const input = document.body.querySelector('[data-testid="scale-input-number"]');
        expect(input?.getAttribute('aria-label')).toBe('Nombre de points par portée');
      });
