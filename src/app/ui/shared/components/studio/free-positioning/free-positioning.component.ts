...
import {
  GetSectionOutput,
  Task
} from '@core/services/worker_python/tasks/types';
...
export class FreePositioningComponent implements OnDestroy {
  ...
  recreatePlots = debounce(async () => {
    ...
    const litData = await this.workerPythonService.runTask(
    ...
    await this.createPlot(litData.result, startSupport, 'face', supports);
    await this.createPlot(litData.result, startSupport, 'profile', supports);
    this.isLoading.set(false);
  }, DEBOUNCED_REFRESH_STUDIO_DELAY);

  /**
   * Initializes watchers for plot updates
   */
  private initializePlotWatcher(): void {
    combineLatest([
      toObservable(this.plotService.plotOptions),
      toObservable(this.plotService.workerReady),
      toObservable(this.plotService.litData)
    ])
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(async ([plotOptions, workerReady, litData]) => {
    ...
    combineLatest([
      ...
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(() => {
        this.debounceUpdateSelectedPositionMarkers();
      });
  }
  ...
  private handleClick(
    ...
    if (
      !(evt.target as any)?.className?.baseVal?.includes('drag') &&
      (evt.target as any).tagName !== 'CANVAS'
    ) {
      return;
    }
    ...
    const positions = this.obstacleFormService.positions
      .value as ObstaclePosition[];
    const previousSelectedObstacle = positions.find(
      (o, index) => index === previousSelected
    );
    if (!previousSelectedObstacle) return;
    const newObstacle: ObstaclePosition =
      type === 'profile'
        ? {
    ...
    const positionGroup =
      this.obstacleFormService.positions.at(previousSelected);
    if (positionGroup) {
  ...
  private updateSelectedPositionMarkers(): void {
    const obstaclesPoints = this.obstacleFormService.positions
      .value as ObstaclePosition[];
  ...
  private getAnnotations(
    obstaclesPoints: ObstaclePosition[],
    side: Side
  ): PlotAnnotation[] {
    ...
    obstaclesPoints.forEach((position, index) => {
      ...
      annotations.push({
        ...
        font: {
          size: 30,
          color:
            index === this.obstaclesService.currentPointIndex()
              ? 'red'
              : 'black'
        }
      });
  ...
  private updateMarkerForPlot(
    ...
    if (!plot) return;
    const layoutWithExtras = (plot as PlotElement)
      ._fullLayout as PlotLayoutWithExtras;
  ...
  async createPlot(
    ...
    try {
      ...
      const annotations = this.getAnnotations(
        this.obstacleFormService.form.get('positions')
          ?.value as ObstaclePosition[],
        side
      );
