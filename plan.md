...
| ~~Sélecteurs globaux SCSS dans composants~~ | ~~22 instances~~ | ✅ CORRIGÉ (Étape H — `:host element {}`/`& element {}`) |
| ~~Noms de classes non-BEM~~ | ~~6 fichiers~~ | ✅ CORRIGÉ (Étape H — camelCase→kebab-case) |
| ~~Imbrication SCSS profonde (6+)~~ | ~~5 fichiers~~ | ✅ CORRIGÉ (Étape H — max 5 niveaux) |
...
#### Étape H — BEM SCSS (imbrication, classes, sélecteurs) ✅ TERMINÉE (2026-03-19)

> **3 groupes de violations** identifiés par analyse automatisée. Seuil de violation : profondeur ≥6 (max 5 niveaux acceptés).

68. ✅ **Imbrication profonde réduite** (3 fichiers) :
    - `papoto.component.scss` — bloc `.app-icon` extrait (profondeur 7 → ≤5)
    - `side-tabs.component.scss` — `&.active { &:hover {} }` fusionné en `&.active:hover {}` (profondeur 6 → 5)
    - `obstaclesForm.component.scss` — `&__term`/`&__def` extrait de profondeur 6 vers `&__sticky__results__grid__item { &__term {} &__def {} }` (profondeur 3)

69. ✅ **Classes non-BEM converties camelCase → kebab-case** — SCSS + HTML synchronisés (7 fichiers-paires) :
    - `papoto` : `leftSupport`→`left-support`, `spanLength`→`span-length`, `MED/HL/H1…VR`→kebab, `incertainParam`→`incertain-param` ; grid-template-areas mis à jour
    - `parameter-calculation-15-without-wind` : 11 classes grid camelCase → kebab
    - `temperature-calculation` : 12 classes grid camelCase → kebab
    - `vtl-and-guying` : `tensionInGuy`, `guyAngle`, `chargeVUnderConsole`, `chargeHUnderConsole`, `chargeLIfPulley` → kebab
    - `sidebar` : `stelBar-header/main/footer` → `stel-bar-header/main/footer`
    - `side-tabs` : `side-tab__header__btn__bxsh/label/icon` → `side-tab__btn-shadow/label/icon`
    - `sidebar.component.spec.ts` — sélecteurs CSS mis à jour en conséquence

70. ✅ **Sélecteurs d'éléments HTML** convertis en `:host element {}` ou `& element {}` (28 fichiers SCSS) :
    - Racine → `:host` : `study` (h3), `logged-layout` (main), `init` (hr), `free-positioning` (dl/dd), `supportsTable` (input/td), `manualSection` (dt), `topbar` (header), `tag` (p), `card-study` (article/h3/dl/dt/dd/ul/button)
    - Imbriqués → `& element` : `header` (dt/dd/label), `climate` (input), `span` (label), `sectionsTab` (dl/dt/dd), `study-header` (p/app-icon), `field-datas` (label), `loads-table` (dt/dd/span), `initialConditionModal` (dt/input), `card-info` (h2), `field-measuring` (button), `top-toolbar` (label), `scale-view` (label), `papoto` (dt/dd), `vtl-and-guying` (dt/p), `sidebar` (hr), `obstaclesForm` (input)

##### Definition of Done — Étape H

- [x] 0 fichier avec imbrication ≥6 niveaux
- [x] 0 classe camelCase/PascalCase dans les SCSS de composants
- [x] 0 sélecteur d'élément HTML nu à la racine d'un composant SCSS
- [x] `npm run build:fr` — 0 erreurs (216 fichiers)
- [x] `npm run test -- --run` — 97 suites, 1967 tests pass
- [x] `npm run lint-check` — 0 erreurs, 109 warnings (i18n)
- [x] Rendu visuel identique (`:host element {}` produit le même CSS scopé qu'`element[_ngcontent-xxx] {}`)
