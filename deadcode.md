...
## 12. `getCachedAppVersion` + `areVersionsEqual` + `getOrMigrateAppVersionCacheEntry` + `LEGACY_APP_VERSION_CACHE_KEY` — `core/services/worker_update/service-worker.ts`
...
## 13. `postMessageToAllClients` — `core/services/worker_update/service-worker.ts`
...
## 14. `checkIfAppInstalled` — `core/services/worker_update/service-worker.ts`
...
## 15. Duplicate `install_complete` message — `installApp()` in `service-worker.ts` (BUG FIX)
...
## 16. `lodash.isEqual` import — `core/services/worker_update/worker_update.service.ts` (CLEANUP)
...
## 17. `isFirstUseOffline` signal — `core/services/auth/auth.service.ts`
...
## 18. `OidcClaims` type alias — `core/services/auth/auth.service.ts` (CLEANUP)
...
## 19. `checkIfAppInstalled` — `core/services/worker_update/service-worker.ts`
...
## 20. `noCacheHeaders` function — `core/services/worker_update/service-worker.ts` (CLEANUP)
...
## 21. `export type { AppVersion }` re-export — `core/services/worker_update/worker_update.service.ts` (CLEANUP)
...
## 22. `createUser()` + `validateEmail()` + `user$` + `userSubject` — `core/services/user/user.service.ts`
...
## 23. `mockCurrentVersion` + `mockLatestVersion` dev constants — `core/services/worker_update/worker_update.service.ts`
