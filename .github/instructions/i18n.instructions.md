...
`common.*` is the shared/deduplication namespace for translations — the i18n equivalent of a
"shared" module. Any generic, reused-as-is text (button labels, verbs, statuses: "Calculate",
"Save", "Cancel"...) belongs there, not duplicated under a feature-specific key.

- Before adding a new key, **search both `en.json` and `fr.json` for the exact English/French
  text** you're about to add (not just the key name) — a duplicate is often hiding under a
  different feature-specific key (e.g. `field-measuring.papoto.calculate-button: "Calculate"`
  duplicating `common.calculate: "Calculate"`).
- If the exact same text already exists anywhere in `common.*`, **reuse that key** — do not create
  a feature-specific one, even if the new usage is in an unrelated feature.
- If you find the *same generic text* duplicated under two or more feature-specific keys (a
  pre-existing duplicate, not one you're adding), **fix it**: move it to `common.*` (reuse an
  existing entry or add one if missing), update every template/TS usage to point at
  `common.<key>`, then delete the now-unused feature-specific key(s) from both `en.json` and
  `fr.json`.
- Do not move text to `common.*` if it is only coincidentally similar — only fully identical,
  generic, context-free strings qualify (e.g. a specific "Calculate setting parameter at 15°C"
  button stays feature-specific; a plain "Calculate" button does not).
...
<!-- Third-party single-brace placeholders (PrimeNG paginator) pass through the default
     transpiler untouched — it only interpolates {{ }} — so use the pipe here too. -->
<p-paginator [currentPageReportTemplate]="'studies.table.pagination-report' | transloco" />
```
