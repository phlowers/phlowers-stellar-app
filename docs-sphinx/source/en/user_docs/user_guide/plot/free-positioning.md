...
The **free positioning** mode lets you place elements ({doc}`obstacles <../obstacles>`,
floor, loads, distance markers) on a **single, stable span** without the plot
moving or the reference span changing under your feet. To make this possible,
the mode **freezes** the current span and every control that could change it.

## Enabling free positioning
...
- When the switch is turned **on**:
  - The span currently **selected in the tab** (its span dropdown) is **captured
    once** and becomes the *frozen span* for the whole free positioning session.
    It does not matter which span the studio plot was last zoomed to — the tab
    selection wins.
...
### Coordinate frame on the obstacle tab
...
distance type**. When free positioning is turned on, these three obstacle form
fields are forced to those values and disabled for the whole session.
...
- The existing point coordinates are **not converted** between frames: they
  are reinterpreted in the forced frame. Review the displayed positions after
  the warning.
...
while free positioning is on. The frozen span stays constant for the whole
session.

## Changing the span

Because the span is frozen, the span cannot be changed while free positioning is
on. To work on a different span:
...
and the 3D camera position — is **restored**, so you land back exactly where you
were before entering the mode. This happens whichever way the mode is turned off
(toggle switch, tab change, or automatic exit).

On the **obstacle** and **floor** tabs, free positioning also turns **off by
itself** when the last editable point is removed (the toggle switch flips back
off). There is no longer anything to position, and the switch would otherwise be
left disabled with no way to turn the mode off from that tab — the switch
requires at least one point to be interactive.
