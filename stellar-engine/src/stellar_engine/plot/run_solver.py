...
def change_state(
    ...
    # Adjustment is only run once at study initialization (see
    # stellar_engine.core.study.initialize_study). It must never be re-run
    # here: mechaphlowers 0.11.1+ invalidates the warm-start memento and the
    # load-merge cache internally (SectionStudy.set_loads / BalanceModel.reset),
    # so change_state no longer needs a manual reset workaround.
    if reload:
